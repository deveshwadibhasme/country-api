...
const CountryContainer = ({query,region}) => {
    ...
    return (
        CountryData === null ? <Loading/> : <div className='grid grid-cols-[repeat(2,180px)] md:grid-cols-[repeat(4,280px)] mx-[auto] my-[0] max-w-[1160px] justify-center w-full gap-[10px]'>
            {checkEmpty() ? 
            CountryData.filter((eve)=>eve.name.common.toLowerCase().includes(query)).map((country) => {
             ...
             : 
                regionData.filter((eve)=>eve.name.common.toLowerCase().includes(query)).map((country) => {
                    return <CountryCard key = {country.name.common} name={country.name.common} flag={country.flags.svg} population={(country.population).toLocaleString('en-IN')} region={country.region} capital={country.capital} />
                })
            }
        </div>
